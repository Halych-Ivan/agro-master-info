...
@section('content')
    ...
    <div class="">
        <form action="{{$program->exists ? route('admin.programs.update', $program->id) : route('admin.programs.store')}}"
            ...
            <x-form.file src="{{ old('file', $program->file) }}" name="file" title="Виберіть файл"></x-form.file>
            <x-form.file src="{{ old('image', $program->image) }}" name="image" title="Виберіть картинку" type="img"></x-form.file>
