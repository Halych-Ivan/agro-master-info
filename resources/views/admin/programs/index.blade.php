...
@section('content')
    ...
    <x-__i.admin-icons resource="programs" id=""></x-__i.admin-icons>
            ...
            @foreach($programs as $program)
                <tr>
                    ...
                    <td class="text-center">
                        <img src="{{ asset('/images/programs/'.$program->image) }}" alt="" height="100">
                    </td>
                    <td class="text-center">
                        @if($program->file)
                            <b><a href="{{ asset('uploads/programs/'.$program->file) }}">Переглянути</a></b>
                        @else
                            відсутній
                        @endif
                    </td>
                    <td><b>{{$program->info}}</b></td>
                    <td class="text-center"><x-__i.admin-icons resource="programs" id="{{$program->id}}"></x-__i.admin-icons></td>
                </tr>
            @endforeach
            </tbody>
        </table>
